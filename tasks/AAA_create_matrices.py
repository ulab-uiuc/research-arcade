...
def convert_hierarchical_json_to_graph(paper_data):
    ...
    # Add title as a special section node (optional)
    if 'title_embedding' in paper_data:
        section_idx = len(node_mappings['section'])
        node_mappings['section']['title'] = section_idx
        node_features['section'].append(paper_data['title_embedding'])
    ...
    # Define expected edge types (matching training code expectations + section edges)
    expected_edge_types = [
