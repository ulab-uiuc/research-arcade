...
class CSVArxivParagraphFigure:
    def __init__(self, csv_dir: str):
        ...
        if not os.path.exists(csv_path):
            self.create_paragraph_figures_table()

    def create_paragraph_figures_table(self):
        df = pd.DataFrame(columns=[
            'id', 'paragraph_id', 'figure_id',
            'paper_arxiv_id', 'paper_section_id'
        ])
        df.to_csv(self.csv_path, index=False)
        print(f"Created paragraph_figures CSV at {self.csv_path}")
    ...
    def insert_paragraph_figure_table(self, paragraph_id, figure_id, paper_arxiv_id, paper_section_id):
        ...
        new_row = pd.DataFrame([{
            ...
            'figure_id': figure_id, 'paper_arxiv_id': paper_arxiv_id,
    ...
    def get_all_paragraph_figures(self):
        df = self._load_data()
    ...
    def delete_paragraph_figure_by_paragraph_id(self, paragraph_id: int) -> int:
    ...
    def construct_table_from_csv(self, csv_file):
        """
        Construct the paragraph-figure relationships from an external CSV file.
        
        Args:
            csv_file: Path to the CSV file
            
        Expected CSV format:
            - Required columns: paragraph_id, figure_id,
            paper_arxiv_id, paper_section_id
        ...
        try:
            ...
            required_cols = ['paragraph_id', 'figure_id',
            'paper_arxiv_id', 'paper_section_id']
            ...
            # Note: Not filtering duplicates as this table allows multiple figures per paragraph

            # Ensure correct column order
            external_df = external_df[['id', 'paragraph_id', 'figure_id',
            'paper_arxiv_id', 'paper_section_id']]
            ...
            print(f"Successfully imported {len(external_df)} paragraph-figure relationships from {csv_file}")
            return True
            
        except Exception as e:
            print(f"Error importing paragraph-figure relationships from CSV: {e}")
            return False


    def construct_table_from_json(self, json_file):
        """
        Construct the paragraph-figure relationships from an external JSON file.
        ...
        try:
            ...
            if isinstance(json_data, dict):
                if 'paragraph_figures' in json_data:
                    relations_list = json_data['paragraph_figures']
                else:
            ...
            if not relations_list:
                print("Error: No paragraph-figure data found in JSON file")
                return False
            ...
            print(f"Successfully imported {len(external_df)} paragraph-figure relationships from {json_file}")
            return True
